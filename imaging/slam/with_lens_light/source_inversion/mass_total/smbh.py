...
setup_mass = al.SetupMassTotal(mass_prior_model=al.mp.EllIsothermal)
...
default of an `EllPowerLaw` in this example.
...
setup_mass = al.SetupMassTotal(mass_prior_model=al.mp.EllPowerLaw)

setup_smbh = al.SetupSMBH(smbh_centre_fixed=True)

pipeline_mass = al.SLaMPipelineMass(setup_mass=setup_mass, setup_smbh=setup_smbh)
...
from imaging.slam.with_lens_light import (
    source__parametric,
    mass__total,
    light__parametric,
)
from imaging.slam.with_lens_light import source_inversion
