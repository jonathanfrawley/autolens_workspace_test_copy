...
"""
path_prefix = path.join("imaging", "slam", "mass_total__source_inversion", "preloads")
...
We use the `Preload` object to preload the maximum likelihood pixelization of the SOURCE INVERSION PIPELINE, such that 
it is not recalculated every iteration of the log likelihood function and therefore speeding up the model-fit. This 
uses the result of the SOURCE INVERSION PIPELINE.
"""
analysis = al.AnalysisImaging(
    dataset=masked_imaging,
    hyper_result=source_inversion_results.last.hyper,
    preloads=al.Preloads.setup(
        pixelization=True, result=source_inversion_results.last.hyper
    ),
)
