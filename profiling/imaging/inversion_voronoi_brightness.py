...
source_galaxy = al.Galaxy(
    ...
    hyper_model_image=masked_imaging.image,  # Used by the KMeans
    hyper_galaxy_image=masked_imaging.image,  # Used by the KMeans
)
...
"""
The clustering weight map is used to weight the KMeans clustering algorithm. It is derived from a model=image of the
source galaxy, and therefore has high values (near 1.0) where the source light is contained and low values (near 0.0)
where it does not.

Thus, because the weight map has high values where the source is located, it can be used by the Weighted KMeans
algorithm to cluster points around the source.
...
"""
This calls the weighted KMeans clustering algorithm, which computes a set of clusters (that represent source pixels)
from the image-plane grid of coordinates and the cluster weight map.

https://github.com/Jammy2211/PyAutoArray/blob/master/autoarray/structures/grids/grids.py

The method is `from_total_pixels_grid_and_weight_map`

The KMeans is called as follows:
...
Note that I set `n_iter=1` and `maxiter=5` which speed the KMeans up. 

For higher values of `niter` the KMeans algorithm would be performed repeated times, and choose the KMeans 
clustering that minimizes its merit function. For lensing, the KMeans that minimizes its merit function is not 
necessarily the kmeans that best fits the lensing data, so doing more than one KMeans is somewhat pointless.

`maxiter` is the number of cluster relocations and reallocations performed by the KMeans (until it reaches a solution 
where recopuing the cluster centres does not change the solution). 5 is low, but again we don't typically need the 
'best' KMeans solution to get a good solution for lensing.
"""
for i in range(repeats):
    sparse_grid = pixelization.sparse_grid_from_grid(
        grid=masked_imaging.grid, hyper_image=cluster_weight_map
    )
diff = time.time() - start
...
"""
This repeats the KMeans but uses it to compute the source-pixel (y,x) centres. The run time is already captured by the
KMeans call above so I don't time it.
"""
