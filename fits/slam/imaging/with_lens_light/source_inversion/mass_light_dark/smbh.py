...
where in the final phase of the pipeline:
 ...
 - The lens `Galaxy`'s `LightProfile`'s are an `EllipticalSersic` + `EllipticalExponential`, representing
   a bulge + disk model.
...
For this runner the `SLaMPipelineLightParametric` customizes:
 ...
 - If the disk is an `EllipticalExponential` or `EllipticalSersic`.
...
from fits.slam.imaging.with_lens_light.pipelines import source__parametric, mass__light_dark, light__parametric, \
    source__inversion
